...
class acf_field_icomoon_base extends acf_field {
	...
	public function parse_css() {
		...
		$out = [];
        array_shift(essential_themify_icons());
		
		foreach ( essential_themify_icons() as $cs ) {
	         
			 $id  = str_replace(' ', '-', $cs);
			
			$out[] = array( 'id' => strtolower( $id), 'text' => $cs );
		}
		return $out;
	}
	...
	public function display_css() {
		 ...
		 */
		$font_name = apply_filters( 'saasland_font_family_name', $font_name );
		$font_urls = array(
			'woff2' => drdt_th_core()->css. '/fonts/themify.woff?ousyjt',
			'ttf'  => drdt_th_core()->css. '/fonts/themify.ttf?ousyjt',
			'woff' => drdt_th_core()->css. '/fonts/themify.woff?ousyjt',
			'svg'  => drdt_th_core()->css. '/fonts/themify.svg?ousyjt#themify'
		);
		/**
	...
	function input_admin_enqueue_scripts() {
		...
		$css_file = drdt_th_core()->css. '/themify-icons.css';
		/**
